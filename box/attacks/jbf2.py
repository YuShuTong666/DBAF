...
class JBF2_HSJA(MinimizationAttack):
    ...
    def approximate_gradients(
        ...
        if self.constraint == "l2":
            ...
            rv = self.jbf_generator(originals, x_advs, noise_shape)
    ...
    def jbf_generator(self, originals, x_advs, noise_shape):
        ...
        for i in range(N):
            ...
            for j in range(noise_shape[0]):
                #features = torch.from_numpy(np.random.normal(0., 1., (n * n * 3, 1))).to(torch.float32).cuda()
                features = np.random.normal(0., 1., (n // 4, n // 4, 3)).astype(np.float32)
                features = transform.resize(features, (n, n, 3))
                layers = self.args.layers
