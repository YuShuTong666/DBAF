...
class SurFree(MinimizationAttack):
    ...
    def run(
        ...
        **kwargs: Any,
        ...
        for _ in range(self._steps):
            ...
            mse = self.compute_mse(originals.raw, best_advs.raw)
            print(mse)
            self.mses[-1].append(mse)
            self.queries[-1].append(self._nqueries[0])
    ...
    def _get_candidates(self, originals: ep.Tensor, best_advs: ep.Tensor) -> ep.Tensor:
        ...
        if self.with_interpolation:
            epsilons = ep.concatenate((epsilons, epsilons[0].expand_dims(0) / 2), axis=0)
    ...
    def _get_evolution_function(self, originals: ep.Tensor, best_advs: ep.Tensor, direction_2: ep.Tensor) -> Callable[
        [ep.Tensor], ep.Tensor]:
        ...
        return lambda theta: (
                originals + self._add_step_in_circular_direction(direction_1, direction_2, distances, theta)).clip(0, 1)
    ...
    def _add_step_in_circular_direction(self, direction1: ep.Tensor, direction2: ep.Tensor, r: ep.Tensor,
                                        degree: ep.Tensor) -> ep.Tensor:
...
class Basis:
    ...
    def get_vector(self, ortho_with: Optional[Dict] = None, bounds: Tuple[float, float] = (0, 1)) -> ep.Tensor:
        ...
        vectors = [
            #self._get_vector_i_dct(i, ortho_with[i], bounds).expand_dims(0)
            self._get_vector_i_dct(i, bounds).expand_dims(0)
            for i in range(len(self._originals))
        ]
        return ep.concatenate(vectors, axis=0)

    def get_vector_i(self, index: int, ortho_with: Optional[ep.Tensor] = None,
                     bounds: Tuple[float, float] = (0, 1)) -> ep.Tensor:
