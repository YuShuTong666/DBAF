...
class HSJA_M(MinimizationAttack):
        ...
        **kwargs: Any,
        ...
        for step in range(self.steps):
            ...
            gradients = self.approximate_gradients(
                self._is_adversarial, x_advs, num_gradient_estimation_steps, delta, last
            )
            last = gradients.raw
            ...
            #print("Queries: ", self._nqueries.values(), end=" ")
    ...
    def approximate_gradients(
        ...
        if self.constraint == "l2":
            if last is None:
                ...
                rv = 0.1 * ep.normal(x_advs, noise_shape) + \
                     0.9 * ep.astensor(torch.normal(mean=last.repeat(steps, 1, 1, 1, 1)))
