...
class SurFree(Attack):
    ...
    def attack_targeted(self, x, y, x_adv, args):
        ...
        if args.random_init:
            ...
            flag = False
            for i in range(2000):
                x_adv_init = torch.clamp( torch.randn_like(x_adv_init) * random.random() + random.random(), 0, 1)
                is_adv, is_cache = self.is_adversarial(x_adv_init, y, targeted=True)
                if is_adv == 1:
                    print("找到对抗的i是：", end='')
                    print(i)
                    print(y)
            ...
            if flag:
                ...
                #norm_dist = torch.linalg.norm(x_adv - x) / (x.shape[-1] * x.shape[-2] * x.shape[-3]) ** 0.5
                norm_dist = self.compute_mse(x, x_adv)
                print("Init dist: ", end='')
                print(norm_dist)
                return x_adv
        #norm_dist = torch.linalg.norm(x_adv - x) / (x.shape[-1] * x.shape[-2] * x.shape[-3]) ** 0.5
        ...
        print("Init dist: ", end='')
        print(norm_dist)
        x_adv = self.binary_search_to_boundary(x, y, x_adv, targeted=True)
        #norm_dist = torch.linalg.norm(x_adv - x) / (x.shape[-1] * x.shape[-2] * x.shape[-3]) ** 0.5
        norm_dist = self.compute_mse(x, x_adv)
        print("Init dist after bs: ", end='')
        print(norm_dist)
        if norm_dist < 0.005:
            return x_adv
        explored_orthogonal_directions = ((x_adv - x) / torch.linalg.norm(x_adv - x))
        ...
        if self.attack_config["adaptive"]["bs_min_angle"]:
            ...
            theta_min = args.theta_min #1.7e-5 #0
        ...
        # Attack
        #pbar = tqdm(range(self.attack_config["max_iter"]), colour="red")
        pbar = tqdm(range(5000), colour="red")
        for t in pbar:
            # Get new orthogonal direction and corresponding best angle for a circular step
            epsilon = 0
            while epsilon == 0:
                ...
                for coeff in coefficients:
                    possible_best_epsilon = coeff * theta_max
                    x_evolved = evolution_function(possible_best_epsilon)
                    is_adv, is_cache = self.is_adversarial(x_evolved, y, targeted=True)
                    if self._model.total > 5000:
                        return x_adv
                    if is_cache[0] and step_attempts < self.attack_config["adaptive"]["step_max_attempts"]:
                        rollback = True
                        print("Step movement failure. Rolling back.", coeff * theta_max, step_attempts)
            ...
            if norm_dist < 0.005 or self._model.total > 5000:
                return x_adv
