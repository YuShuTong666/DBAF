...
class DBAGP(Attack):
    ...
    def attack_targeted(self, x, y, x_adv, args):
        ...
        # attack
        pbar = tqdm(range(self.attack_config["max_iter"]))
        self.history_info = deque(maxlen=self.attack_config["max_history"])
        for t in pbar:
            # 1. compute new delta
            if not self.attack_config["adaptive"]["bs_grad_var"]:
            ...
            # time prior
            val_grads = []
            for pre_adv, pre_grad in self.history_info:
                cos = torch.cosine_similarity(pre_grad.view(-1), grad.view(-1), dim=0)
                print('cos:{}'.format(cos))
                q = 0.005
                if torch.norm(pre_adv - x_adv, p=2) < 0.15 and cos > q:
                    val_grads.append(pre_grad)
            if len(val_grads) > 0:
                pre_grad = torch.mean(torch.stack(val_grads), dim=0).squeeze()
                pre_grad /= torch.linalg.norm(pre_grad)
                print('pre_grad:{}'.format(pre_grad.shape))
                grad = 2 * grad - pre_grad
                grad /= torch.linalg.norm(grad)
            self.history_info.append((grad, x_adv))
