...
def imagenet_target_attack(args, N_img, model_config):
    ...
    for _, (x, y) in enumerate(loader):
        x = x.cuda()
        y = y.cuda()
        if y[0] != y[1] and torch.argmax(model.f(x[0].unsqueeze(0)).squeeze()) == y[0] and torch.argmax(
                model.f(x[1].unsqueeze(0)).squeeze()) == y[1]:
...
def main(args):
    ...
    if not args.disable_logging:
        ...
        logging.basicConfig(filename=os.path.join(writer.log_dir,
    ...
    # Load model.
    if args.celeba:
        src_images, src_labels, tgt_images, tgt_labels, model = celeba_target_attack(args=args_dba,
                                                                                     N_img=args_dba.n_img,
                                                                                     model_config=model_config)
    else:
        src_images, src_labels, tgt_images, tgt_labels, model = imagenet_target_attack(args=args_dba,
                                                                                       N_img=args_dba.n_img,
                                                                                       model_config=model_config)
    model.eval()
    model.to("cuda")
